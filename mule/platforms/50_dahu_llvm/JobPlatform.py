...
def jobscript_get_header(jg : JobGeneration):
    ...
    content = """#! /bin/bash
#OAR -n sweet_job
#OAR -l /nodes="""+str(p.num_ranks)+ """/core="""+str(p.num_cores_per_node)+""",walltime="""+time_str+"""
...
def jobscript_get_exec_command(jg : JobGeneration):
    ...
    taskset = ""
    #####taskset = "taskset -c "
    #####for ithread in range(p.num_threads_per_rank):
    #####    if ithread <= 15:
    #####        taskset += str(2 * ithread) + ","
    #####    else:
    #####        taskset += str(2 * (ithread - 16) + 1) + ","
    #####taskset = taskset[:-1] + " "

    places = ""
    for ithread in range(p.num_threads_per_rank):
        if ithread <= 15:
            places += str(2 * ithread) + ","
        else:
            places += str(2 * (ithread - 16) + 1) + ","
    places = places[:-1]
...
EXEC=\""""+ taskset + jg.compile.getProgramPath()+"""\"
