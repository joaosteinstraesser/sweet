	...
	///						);
			...
			else if (simVars->iodata.output_file_mode == "bin")
			{
				...
				if (iteration_id == 0)
				{
					...
					if (base_solution == "fine" || simVars->xbraid.xbraid_spectral_ref == this->sphereDataConfig[0]->spectral_modes_n_max)
					{
						ref_data[ivar].file_read_binary_spectral(buffer2);
					}
					// interpolate (restrict) to finest level
					else if (simVars->xbraid.xbraid_spectral_ref > this->sphereDataConfig[0]->spectral_modes_n_max)
					{
						SphereData_Spectral ref_data_finer(this->sphereDataConfig.back());
						ref_data_finer.file_read_binary_spectral(buffer2);
						ref_data[ivar] = ref_data[ivar].restrict(ref_data_finer);
					}
					// interpolate (pad zeros) to finest level
					else
					{
						SphereData_Spectral ref_data_finer(this->sphereDataConfig.back());
						ref_data_finer.file_read_binary_spectral(buffer2);
						ref_data[ivar] = ref_data[ivar].pad_zeros(ref_data_finer);
					}
		...
		for (int ivar = 0; ivar < nvar; ivar++)
		{
			...
			SphereData_Spectral sol = *i_data->get_pointer_to_data_SphereData_Spectral()->simfields[ivar];
			SphereData_Spectral ref = *pint_data_ref->get_pointer_to_data_SphereData_Spectral()->simfields[ivar];
			SphereData_Physical sol_phys = sol.toPhys();
			SphereData_Physical ref_phys = ref.toPhys();

			if (prog_phi)
			{
				sol_phys = sol_phys + simVars->sim.h0 * simVars->sim.gravitation;
				ref_phys = ref_phys + simVars->sim.h0 * simVars->sim.gravitation;
				sol.loadSphereDataPhysical(sol_phys);
				ref.loadSphereDataPhysical(ref_phys);
			}


			SphereData_Spectral diff_spectral = sol - ref;
			SphereData_Physical diff = sol_phys - ref_phys;
...
//////////////			SphereData_Physical diff = i_data->get_pointer_to_data_SphereData_Spectral()->simfields[ivar]->toPhys() -
			...
			if (base_solution == "fine" || simVars->xbraid.xbraid_spectral_ref == this->sphereDataConfig[0]->spectral_modes_n_max)
			{
				resx_data_ref = resx_data;
				resy_data_ref = resy_data;
			}
			else
			{
				resx_data_ref = simVars->xbraid.xbraid_spectral_ref;
				resy_data_ref = simVars->xbraid.xbraid_spectral_ref;
			}
			double ref_L1 = ref_phys.physical_reduce_norm1() / (resx_data_ref * resy_data_ref);
			double ref_L2 = ref_phys.physical_reduce_norm2() / std::sqrt(resx_data_ref * resy_data_ref);
			double ref_Linf = ref_phys.physical_reduce_max_abs();
			if (ref_L1 > 1e-14)
				err_L1 /= ref_L1;
			if (ref_L2 > 1e-14)
				err_L2 /= ref_L2;
			...
			for (std::vector<std::size_t>::iterator it = rnorms.begin(); it != rnorms.end(); it++)
			{
				...
				double norm_diff = std::sqrt(diff_spectral.spectral_reduce_max_abs(*it));
				double norm_ref = std::sqrt(ref.spectral_reduce_max_abs(*it));
