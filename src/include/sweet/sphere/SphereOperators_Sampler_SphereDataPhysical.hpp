...
public:
	const SphereData_Physical bi_interp_scalar_ret_phys(
			const SphereData_Physical &i_data,				///< sampling data
	...
	)
	{
		...
		bi_interp_scalar(i_data, i_pos_x, i_pos_y, out, order, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		return out;
	}

//////public:
//////	SphereData_Spectral bi_interp_scalar(
//////			const SphereData_Spectral &i_data,			///< sampling data
//////
//////			const ScalarDataArray &i_pos_x,		///< x positions of interpolation points
//////			const ScalarDataArray &i_pos_y,		///< y positions of interpolation points
//////
...
//////	{
//////
//////		SphereData_Physical i_data_phys = i_data.toPhys();
//////		SphereData_Spectral out(i_data.sphereDataConfig);
//////
//////		SphereData_Physical out_phys = bi_interp_scalar(i_data_phys, i_pos_x, i_pos_y, order, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
//////
//////		out.loadSphereDataPhysical(out_phys);
//////
//////		return out;
//////
//////	}
...
public:
	void bi_interp_scalar(
			...
			const SphereData_Physical &o_data,			///< sampling data
	...
	)
	{
		...
		assert(i_pos_x.number_of_elements == (std::size_t)o_data.sphereDataConfig->physical_array_data_number_of_elements);
			...
			bicubic_scalar_NEW2(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		///else if (order == 34)
		///	bicubic_scalar_NEW4(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		///else if (order == 36)
		///	bicubic_scalar_NEW6(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		///else if (order == 38)
		///	bicubic_scalar_NEW8(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		else if (order == -3)
			bicubic_scalar(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		///else if (order == 5)
		///	biquintic_scalar(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		///else if (order == 55)
		///	biquintic_scalar2(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		///else if (order == 555)
		///	biquintic_scalar3(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		///else if (order == -5)
		///	biquintic_scalar_Lagrangian(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		///else if (order == -7)
		///	biseptic_scalar_Lagrangian(i_data, i_pos_x, i_pos_y, o_data.physical_space_data, i_velocity_sampling, i_pole_pseudo_points, i_limiter);
		else
...
#pragma omp parallel for
		...
		for (std::size_t pos_idx = 0; pos_idx < i_pos_lon.number_of_elements; pos_idx++)
		{
			...
			for (int kj = 0; kj < 4; kj++)
			{
				...
				assert(cell_rel_x+1.0 >= 1.0 && cell_rel_x+1.0 <= 2.0);
				//q[kj] = interpolation_lagrange_equidistant<4>(p, cell_rel_x+1.0);
				/////////////q[kj] = interpolation_hermite<4, 3, 2>(p, cell_rel_x+1.0, true);

				double x = cell_rel_x + 1.;
				double x0 = 1.;
				double x1 = 2.;
				double p0 = p[1];
				double p1 = p[2];
			...
			assert((phi_lookup_[array_idx_y] >= pos_lat) && (phi_lookup_[array_idx_y+1] <= pos_lat));
			//double value = interpolation_lagrange_nonequidistant<4>(&phi_lookup_[array_idx_y-1], q, pos_lat);
			///double value = interpolation_hermite<4, 3, 2>(q, pos_lat, false, &phi_lookup_[array_idx_y-1] );


			double y = pos_lat;
			double* yy = &phi_lookup_[array_idx_y-1];
			double y0 = yy[1];
			double y1 = yy[2];
			double q0 = q[1];
			double q1 = q[2];

			double dery0  = (q[2] - q[0]) / (yy[2] - yy[0]);
			double dery1  = (q[3] - q[1]) / (yy[3] - yy[1]);
